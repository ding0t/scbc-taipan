...
function apt_purge_tools(){
	A_TOOLS=(netcat-* aircrack-ng airmon-ng hydra-gtk \
	john johnny hive \
	burp cainandable myheritage \
	wireshark nmap nikto hashcat \
	etherape kismet lcrack ophcrack \
	sl Freeciv zenmap)
    for i in "${A_TOOLS[@]}"; do
        apt purge -y "${i}"
    done
...
function apt_purge_servcies(){
	A_SERVICES=(apache2 dnsmasq mysql-server \
	nmdb php5 postfix postgresql \
	tomcat tomcat6 telnet telnet-server \
	tightvnc-common tightvncserver vnc4server vncserver)
